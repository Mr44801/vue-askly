...
const addQuestion = async () => {
  ...
  try {
    // Moderation vor dem Senden
    const moderation = await moderateQuestion(newQuestionText.value.trim())
    if (moderation.flagged) {
      errorMessage.value = `Deine Frage wurde abgelehnt: ${moderation.reason || 'Unangemessener Inhalt'}`
      return
    }
    ...
    <div class="min-h-screen transition-colors duration-300 bg-white dark:bg-gray-900">
        ...
        <div v-if="isMenuOpen" class="fixed inset-0 z-40 md:hidden" @click="isMenuOpen = false">
          ...
          <nav class="absolute top-0 right-0 w-64 h-full bg-white dark:bg-gray-900 shadow-xl p-6" @click.stop>
            <div class="mt-16 flex flex-col gap-3">
              ...
              <button @click="showLoginModal = true; isMenuOpen = false" class="px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-left font-medium">
                🔐 Anmelden
              </button>
            </div>
          </nav>
      ...
      <main class="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div v-if="currentView === 'home'">
          <HomeView @navigate="(view) => { currentView = view; }" />
        </div>
