...
const handleLogin = async () => {
  ...
  setTimeout(() => {
    if (loginForm.value.email === 'demo@askly.de' && loginForm.value.password === 'demo123') {
      emit('login-success', { username: 'Demo User', email: 'demo@askly.de', role: 'student', isGuest: false })
    }
...
const handleRegister = async () => {
  if (registerForm.value.password !== registerForm.value.passwordConfirm) return
  isLoading.value = true
  setTimeout(() => {
    emit('login-success', { username: registerForm.value.username, email: registerForm.value.email, role: 'student', isGuest: false })
    isLoading.value = false
  }, 800)
}
</script>